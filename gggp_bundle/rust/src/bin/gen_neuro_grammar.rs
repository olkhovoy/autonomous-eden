...
fn main() {
    let mut grammar = build_neuro_grammar(5);
    if let Err(e) = grammar.to_file("../neuro_grammar.cfg") {
        eprintln!("Failed to write neuro_grammar.cfg: {}", e);
        std::process::exit(1);
    }
    println!("Generated neuro_grammar.cfg");
}
