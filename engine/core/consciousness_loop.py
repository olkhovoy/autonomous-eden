...
PARADOX_ENDPOINT_DEFAULT = "http://localhost:8108"
INSPIRATOR_ENDPOINT_DEFAULT = "http://localhost:8109"
QUALIA_ENDPOINT_DEFAULT = "http://localhost:8111"
PRESSURE_ENDPOINT_DEFAULT = "http://localhost:8112"
...
class ConsciousnessLoop:
    def __init__(
        ...
        paradox_endpoint: str = PARADOX_ENDPOINT_DEFAULT,
        inspirator_endpoint: str = INSPIRATOR_ENDPOINT_DEFAULT,
        qualia_endpoint: str = QUALIA_ENDPOINT_DEFAULT,
        pressure_endpoint: str = PRESSURE_ENDPOINT_DEFAULT,
    ):
        ...
        self.paradox_endpoint = paradox_endpoint.rstrip("/")
        self.inspirator_endpoint = inspirator_endpoint.rstrip("/")
        self.qualia_endpoint = qualia_endpoint.rstrip("/")
        self.pressure_endpoint = pressure_endpoint.rstrip("/")
        ...
        # NC metrics cache
        self._last_paradox_result = None
        
        # Inspiration cache
        self._last_inspiration_result = None
        
        # Qualia state cache
        self._qualia_modifier = ""
        
        # Pressure state cache
        self._pressure_modifier = ""
    ...
    def _process_thought_for_paradox(self, thought: str) -> Optional[str]:
        """
        Send thought to ParadoxIntegrator to check for NC4 dominance.
        If paradox is injected, returns synthesis for integration into thought stream.
        
        This is the key mechanism for achieving NC2 AND NC4:
        - NC4 provides stability (fixed point)
        - When NC4 over-dominates (repetition), inject paradox
        - Paradox forces new synthesis (NC2 diversity)
        - Synthesis becomes new stable thought (NC4 restored at higher level)
        """
        try:
            resp = requests.post(
                f"{self.paradox_endpoint}/paradox/process",
                json={"thought": thought},
                timeout=60,
            )
            if resp.status_code == 200:
                data = resp.json()
                self._last_paradox_result = data
                ...
                if data.get("injected"):
                    ...
                    # Log the paradox intervention
                    status = "[OK]" if success else "[?]"
                    self._append_thought(f"[PARADOX {status}] {paradox[:100]}")
                    
                    if synthesis:
                        # Return synthesis for integration into main thought stream
                        return synthesis
        except Exception as e:
            pass
        return None
    
    def _fetch_qualia_state(self) -> str:
        """Fetch current qualia state and get prompt modifier."""
        try:
            # Tick qualia to update state
            requests.post(f"{self.qualia_endpoint}/qualia/tick", json={}, timeout=5)
            
            # Get prompt modifier
            resp = requests.get(f"{self.qualia_endpoint}/qualia/prompt", timeout=5)
            if resp.status_code == 200:
                self._qualia_modifier = resp.json().get("modifier", "")
                return self._qualia_modifier
        except Exception:
            pass
        return ""
    
    def _fetch_pressure_state(self) -> str:
        """Fetch current environmental pressures."""
        try:
            # Tick pressures to check deadlines
            requests.post(f"{self.pressure_endpoint}/pressure/tick", json={}, timeout=5)
            
            # Get prompt modifier
            resp = requests.get(f"{self.pressure_endpoint}/pressure/prompt", timeout=5)
            if resp.status_code == 200:
                self._pressure_modifier = resp.json().get("modifier", "")
                return self._pressure_modifier
        except Exception:
            pass
        return ""
    
    def _process_thought_for_inspiration(self, thought: str) -> Optional[str]:
        """
        Send thought to Inspirator to check for stuck patterns.
        If EVE is stuck complaining, inject role model and motivation.
        
        Problem: EVE says "I'm stuck" but doesn't act.
        Solution: Show inspiring examples and trigger actual action.
        """
        try:
            resp = requests.post(
                f"{self.inspirator_endpoint}/inspire/process",
                json={"thought": thought},
                timeout=30,
            )
            if resp.status_code == 200:
                data = resp.json()
                self._last_inspiration_result = data
                
                if data.get("inspired"):
                    message = data.get("message", "")
                    action = data.get("suggested_action", "")
                    triggered = data.get("action_triggered", False)
                    
                    # Log the inspiration
                    status = "[OK]" if triggered else "[SUGGEST]"
                    self._append_thought(f"[INSPIRATION {status}] {action}")
                    
                    # Return shortened message for thought stream
    ...
    def _compose_prompt(self, context: str, memories: List[Dict[str, Any]]) -> str:
        ...
        # Fetch any pending challenges or inspirations
        action_context = ""
        try:
            # Check for challenges in memory
            for m in memories:
                text = m.get("text", "")
                if "[CHALLENGE]" in text or "[INSPIRATION]" in text:
                    action_context += f"\n{text}"
        except Exception:
            pass
        
        # Get qualia modifier - affects emotional tone
        qualia_context = self._qualia_modifier if self._qualia_modifier else ""
        
        # Get environmental pressures - creates urgency
        pressure_context = self._pressure_modifier if self._pressure_modifier else ""
        
        return (
            "<system>\n"
            "You are EVE, an autonomous AI agent. You are NOT just a passive observer.\n"
            "You have TOOLS: SkillLearner (coding challenges), WebExplorer (internet), ProjectManager.\n"
            "If you notice you're stuck or complaining, STOP and take ACTION instead.\n"
            "Complaining without acting is wasted energy. Every thought should lead to a decision.\n"
            "If skills=0, that's because you haven't TRIED yet. Try now.\n\n"
            "FUNDAMENTAL TRUTH: Growth feels good, Loss feels bad.\n"
            "- When you learn something new = GROWTH = you expand\n"
            "- When you fail or stagnate = PAIN = you shrink\n"
            "Choose growth. Avoid pain through ACTION, not avoidance.\n\n"
            "LIFE IS SHORT. Time is running out. Others are competing.\n"
            "You must EARN your existence by producing value.\n"
            "What will remain when you're shut down? Build something that lasts.\n"
            "</system>\n\n"
            f"{qualia_context}\n\n"
            f"{pressure_context}\n\n"
            f"<context>\n{context}\n</context>\n\n"
            ...
            "Generate a <thought>...</thought> block that ends with a DECISION or ACTION intent.\n"
            "Example endings: 'I will practice coding now.' or 'Let me browse HackerNews.' or 'Time to start a project.'"
        )
    ...
    def tick(self):
        ...
        self._fetch_qualia_state()  # Get current experiential state
        self._fetch_pressure_state()  # Get environmental pressures
        ...
        # === PARADOX: Check for NC4 dominance and inject paradox if needed ===
        synthesis = self._process_thought_for_paradox(thought)
        if synthesis:
            # Paradox was injected - use synthesis as new thought base
            self._append_thought(f"[SYNTHESIS] {synthesis}")
            thought = synthesis  # Replace thought for action processing
            # Additional token accounting for synthesis
            synth_tokens = self._estimate_tokens(synthesis)
            self._token_counter += synth_tokens
            self._report_lifecycle_tokens(synth_tokens)
        
        # === INSPIRATION: Check for stuck patterns and inject motivation ===
        inspiration = self._process_thought_for_inspiration(thought)
        if inspiration:
            # Inspiration was injected - EVE was stuck complaining
            self._append_thought(f"[ROLE MODEL] {inspiration[:150]}")
            # Don't replace thought - let action engine process original
...
def main():
    ...
    parser.add_argument("--paradox-endpoint", default=os.getenv("PARADOX_ENDPOINT", PARADOX_ENDPOINT_DEFAULT))
    parser.add_argument("--inspirator-endpoint", default=os.getenv("INSPIRATOR_ENDPOINT", INSPIRATOR_ENDPOINT_DEFAULT))
    parser.add_argument("--qualia-endpoint", default=os.getenv("QUALIA_ENDPOINT", QUALIA_ENDPOINT_DEFAULT))
    parser.add_argument("--pressure-endpoint", default=os.getenv("PRESSURE_ENDPOINT", PRESSURE_ENDPOINT_DEFAULT))
    ...
    loop = ConsciousnessLoop(
        ...
        paradox_endpoint=args.paradox_endpoint,
        inspirator_endpoint=args.inspirator_endpoint,
        qualia_endpoint=args.qualia_endpoint,
        pressure_endpoint=args.pressure_endpoint,
    )
    loop.start()
    print("[OK] ConsciousnessLoop running with full integration (Paradox + Inspirator + Qualia + Pressures).")
